...
- Default
```shell
./gradlew -p backend-infrastructure clean test
```
- TestContainers
```shell
./gradlew -p backend-infrastructure clean test -Dspring.profiles.active=testcontainers
```
- mongoDB
```shell
./gradlew -p backend-infrastructure clean test -Dspring.profiles.active=mongo
```
